...
public class NURBTest {
	...
	/**
	 * Main loop. Builds the scene and runs the loop itself. Should be
	 * refactored to only include the loop itself.
	 */
	public void loop() {
	...
	private void buildGeometry() {
		...
		Vec3f[] pts = { new Vec3f(-0.5f, -0.5f, -0.5f), new Vec3f(-0.5f, 0.5f, 0), new Vec3f(0.5f, 0.5f, 0),
				new Vec3f(0.5f, -0.5f, -0.5f), new Vec3f(1f, -0.5f, 0), new Vec3f(1f, 0, 0) };

		int[] e = { 0, 1, 2, 3, 4, 5 };
		for (int i = 0; i < e.length - 3; i++) {
			Curve c = new Curve(Curve.array(pts[e[i]], pts[e[i + 1]], pts[e[i + 2]], pts[e[i + 3]]),
					Curve.array(e[i], e[i + 1], e[i + 2], e[i + 3]));
			List<Vec3f> l = c.bezier(0.0625f);
			for (Vec3f v : l) {
